...
```{r}
madden17QB <- read.csv('https://raw.githubusercontent.com/cadentwebb/football/main/Simple%20Dataset/madden17QB.csv?token=AQ3PMS4CKZPUBBGUZDQPXXLAVUW3O')
madden17RB <- read.csv('https://raw.githubusercontent.com/cadentwebb/football/main/Simple%20Dataset/madden17RB.csv?token=AQ3PMS4BY72ZOSABYCI4EFDAVUW5W')
qb_chulls <- merge(x = all_games.merged, y = madden17QB, by = "displayName", all = TRUE)
...
for(i in 1:length(unique(all_games.merged$game_play))){
  library(dplyr)
   ...
   # Get the play I want
    area <- all_games.merged %>% filter(game_play == unique(all_games.merged$game_play)[1]) %>% filter(displayName != "football")
...
qbr2017 <- read_csv("https://raw.githubusercontent.com/cadentwebb/football/main/Simple%20Dataset/2017-2018%20Weekly%20ESPN%20QBR.csv?token=AQ3PMS4VQ4AG72T62TASUATAVUXME")
